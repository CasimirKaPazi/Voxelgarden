...
-- 		Use delay to the check for surrounding nodes.
--		Place wallmonted nodes (as if by a player).
-- Relevant lines are marked with "-- VG" at the end.
-- As soon as this has been added to the engine this file can be removed.

local builtin_shared = {}		-- VG
...
core.register_entity(":__builtin:falling_node", {
			...
			if def.tiles and def.tiles[1] then
				...
				if type(tile) == "table" then
					if tile.animation then
						tile = "("..tile.name..")^[verticalframe:16:1"
					else
						tile = tile.name
					end
				end
				-- Pick only one frame for animated nodes
				if def.drawtype == "torchlike" then  -- VG
					textures = { "("..tile..")^[transformFX", tile }  -- VG
				else  -- VG
					textures = { tile, "("..tile..")^[transformFX" }  -- VG
				end  -- VG
			end
			local vsize
	...
	try_place = function(self, bcp, bcn)
		...
		if def then
			-- Have wallmounted properly placed.		-- VG
			if core.registered_items[self.node.name].paramtype2 == "wallmounted" then		-- VG
				core.place_node(np, self.node)		-- VG
			else		-- VG
...
local function drop_attached_node(p)
	...
	core.remove_node(p)
	if drops[1] == n.name then		-- VG
		-- when there are no drops defined let the node fall down		-- VG
		convert_to_falling_node(p, n)		-- VG
	else		-- VG
...
local function delay_check_for_falling(p)		-- VG
...
function core.check_for_falling(p)		-- VG
	minetest.after(0.1, delay_check_for_falling, p)		-- VG
end		-- VG

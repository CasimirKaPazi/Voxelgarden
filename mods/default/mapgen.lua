	...
	if maxp.y >= 2 and minp.y <= 0 then
		...
		for divz=0,divs-1 do
			...
			for i=0,papyrus_amount do
				...
				if minetest.env:get_node({x=x,y=1,z=z}).name == "default:dirt_with_grass" and
						minetest.env:find_node_near({x=x,y=1,z=z}, 3, "default:water_source") then
					default.make_papyrus({x=x,y=2,z=z}, pr:next(1, 4))
				-- papyrus roots
				elseif minetest.env:get_node({x=x,y=1,z=z}).name == "default:water_source" and
						minetest.env:find_node_near({x=x,y=1,z=z}, 2, "default:dirt_with_grass") then
					minetest.env:set_node({x=x,y=1,z=z}, {name="default:papyrus_roots"})
					default.make_papyrus({x=x,y=2,z=z}, pr:next(1, 4))
				end
			end
		end
		...
		for divz=0,divs-1 do
			...
			for i=0,cactus_amount do
				...
				elseif ground_y and minetest.env:get_node({x=x,y=ground_y,z=z}).name == "default:sand" and
						minetest.env:find_node_near({x=x,y=1,z=z}, 5, "default:desert_sand") then
					default.make_cactus({x=x,y=ground_y+1,z=z}, pr:next(3, 4))
				end
			end
		end
			...
			for i=0,grass_amount do
				...
				if ground_y then
					...
					if minetest.registered_nodes[nn] and
						...
						minetest.env:find_node_near({x=x,y=1,z=z}, 5, "default:jungletree") then
							minetest.env:set_node(p,{name="default:junglegrass"})
						end
					end
				end
